...
export function AuthProvider({ children }) {
  const [auth, setAuth] = useState(() => {
    ...
    const profileRaw = localStorage.getItem("kalashree_student_profile");
    return { token: token || null, profile: profileRaw ? JSON.parse(profileRaw) : null };
  });

  const login = (token, profile) => {
    localStorage.setItem("kalashree_student_token", token);
    localStorage.setItem("kalashree_student_profile", JSON.stringify(profile));
